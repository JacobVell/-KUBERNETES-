...
-масштабирование приложения при росте нагрузки когда нагрузка на приложение превышает 50% (например, по CPU или памяти), контроллер горизонтального автоскейлинга (Horizontal Pod Autoscaler) автоматически увеличивает количество подов, чтобы поддерживать стабильную производительность.

🧱 Компоненты кластера
Компонент	                                Назначение

kubeadm	-                                  Инициализация и управление кластером

kubelet	-                                  Запуск и контроль подов на каждой ноде

kubectl -                                 	CLI для взаимодействия с кластером

Flannel	-                                  Сетевой плагин (CNI) для подов

Dashboard -                              	Веб-интерфейс управления кластером

HPA (Horizontal Pod Autoscaler) -        	Автоматическое масштабирование подов по нагрузке
